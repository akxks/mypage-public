...
$("#notifDesk").click(function() {
  $.ajax({ 
      type: "POST", 
      url: "api/notifread",
      success: function(click){ 
        if (document.getElementById('notificationsDesk').classList.contains("show")) { loadLatestResults("notificationsDesk"); }  
      } 
  }); 
}); 
}); 
...
$(document).ready(function() { 
$("#friendButtonDesk").click(function() { 
  $.ajax({ 
      type: "POST", 
      url: "api/notifread",
      success: function(click){ 
        if (document.getElementById('friendsDesk').classList.contains("show")) { loadLatestResults("friendsDesk");  } 
    } 
  }); 
});
}); 
...
<?php  
  ...
  $idnameNotif = "friendButtonDesk";
  $idnameNotifMenu = "friendsDesk"; 
  $id2nameNotif = "friendsDesk"; 

?>

<button id="<?php echo $idnameNotif; ?>"  onclick="menu('<?php echo $idnameNotifMenu; ?>')" style="padding-top:-2px;font-size:20px;background-color: transparent;color:white;text-shadow: 0px 0px 20px rgba(0, 0, 0, 0.6); float:left;" class="dropbtn"> <?php echo $emoji; ?> <p style='position:absolute;margin-top:-10px;margin-left:15px;font-size:15px;'  class='notiftextColor'> <?php if ($countfriendsreqs != 0) {echo $countfriendsreqs; } ?> </p> </button>
<div class="dropdown" style="z-index:998;"> 
<?php   
if ($countfriendsreqs == 0) { 
    echo '<div id="'.$id2nameNotif.'" class="dropdown-content systemcolor-noborder" style="width: 230px;  height: 100px; overflow: scroll; overflow-x:hidden; ">' ; 
}
else {
    echo '<div id="'.$id2nameNotif.'" class="dropdown-content systemcolor-noborder" style="width: 450px;  height: 400px; overflow: scroll; overflow-x:hidden; " >'  ;
} 
?>  
...
<?php  
  ...
  $idnameNotif = "notifDesk";
  $idnameNotifMenu = "notificationsDesk"; 
  $id2nameNotif = "notificationsDesk"; 

?>

<button id="<?php echo $idnameNotif; ?>" onclick="menu('<?php echo $idnameNotifMenu; ?>')" style="margin-right:20px;font-size:20px;background-color: transparent;color:white;text-shadow: 0px 0px 20px rgba(0, 0, 0, 0.6); float:left;" class="dropbtn">🔔 <p style='position:absolute;margin-top:-10px;margin-left:15px;font-size:15px;' class='notiftextColor '> <?php if ($countnotifs != 0) {echo $countnotifs; } ?> </p> </button>

<div class="dropdown" style="z-index:998;"> 
    <?php   
    if ($countnotifsExist == 0) { 
        echo '<div id="'.$id2nameNotif.'" class="dropdown-content systemcolor-noborder" style="width: 230px; height: 400px; overflow: scroll; overflow-x:hidden; " >' ; 
    }
    else {
        echo '<div id="'.$id2nameNotif.'" class="dropdown-content systemcolor-noborder" style="width: 367px; height: 400px; overflow: scroll; margin-left:-100px; overflow-x:hidden; ">'  ;
    } 
    ?> 
   ...
   <?php 
    ...
    <?php  
     ...
     if (empty($notifications)) { 
        echo ' <a class="hrefColor">'.$lang_notifsfetch_none.'</a> '; 
         ...
         while ($notificationsNo != -1) {    
           ...
           if (str_contains($notification, "mentioned")) {  
              $notif = "". ("$notification");
           } 
     ...
     echo " <hr> </hr> ";
     //echo " <center> <a class='notifsColor'> ☁️ No more notifications </a> </center> " ;   
     echo "</div>"; 
     echo "</div>"; 
